...
//       )
// }
    const [cmsNews, setCmsNews,staticNews] = useState([]);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);

    useEffect(() => {
        const fetchCmsNews = async () => {
            try {
    ...
    // Combine static + CMS news
    const staticArray = Array.isArray(staticNews) ? staticNews : [];
    const cmsArray = Array.isArray(cmsNews) ? cmsNews : [];
    const allNews = [...staticArray, ...cmsArray];
    ...
    return (
        <div className="p-6">
            ...
            {allNews.length === 0 && !loading && <p>No news available.</p>}
            <div>
                                {/*<h3 className="font-semibold mb-4 bg-[rgb(0,175,240)]">CONTENT MANAGEMENT SYSTEM SECTION</h3>*/}
                                 <ul>
                ...
                {allNews.map((item, index) => (
                    <div key={index} className="p-4 border rounded-lg shadow">
                        ...
                        <div className="flex gap-4 text-sm text-gray-500 mt-2">
                            <span className="flex items-center gap-1"><LiaClockSolid /> {item.date}</span>
                            <span className="flex items-center gap-1"><SlLocationPin /> {item.location}</span>
                        </div>
                    </div>
                ))}
            </div>
